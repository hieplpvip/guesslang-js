...
function* runVSCodeHeuristic(modelResults: ModelResult[]) {
  ...
  for (let current of modelResults) {
    ...
    if (currentHighest.confidence - current.confidence >= expectedRelativeConfidence) {
      ...
      if (
        (currentHighest.languageId === 'ts' && current.languageId === 'js') ||
...
async function modelJsonLoaderFunc(): Promise<{ [key: string]: any }> {
...
async function weightsLoaderFunc(): Promise<ArrayBuffer> {
  const fs = await import('fs');
  const path = await import('path');
...
describe('describe', () => {
  const modelOperations = new GuessLang({
    modelJsonLoaderFunc,
    weightsLoaderFunc,
  });

  it('test TypeScript', async () => {
    const result = await modelOperations.runModel(`
